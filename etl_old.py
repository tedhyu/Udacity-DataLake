...
def process_song_data(spark, input_data, output_data):
    ...
    song_data = input_data + 'song_data/A/W/W'
    # read song data file
...
def process_log_data(spark, input_data, output_data):
    ...
    newRow = spark.createDataFrame([ ('Ted', 'Logged In', 'Celeste', 'F', 0, 'Williams', 9.9, 'free', 'LA', 'GET', 'NextSong', 100, 100, 'Think', 200, 10000000, 'bla', 53)], df.columns)
    df = df.union(newRow)
...
def main():
    ...
    # read log data file
    df = spark.read.json(log_data)
    newRow = spark.createDataFrame([ ('Ted', 'Logged In', 'Celeste', 'F', 0, 'Williams', 9.9, 'free', 'LA', 'GET', 'NextSong', 100, 100, 'Shoutsly', 200, 10000000, 'bla', 53)], df.columns)
    df = df.union(newRow)
    ...
    # extract columns for users table    
    users_table = spark.sql("""
        SELECT DISTINCT userID, song, lastName, gender, level
        FROM  log_stage
        WHERE song = 'Shoutsly'
        LIMIT 50
    """).show()
